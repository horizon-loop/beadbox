// useActiveWorkspace — the resolution contract the activity and formulas
// routes depend on.
//
...
// them for an already-verified workspace).
...
function Probe({ workspaces }: { workspaces: Workspace[] }) {
  ...
  return <span data-testid="active">{`${active?.id ?? "none"}|${active?.name ?? ""}`}</span>
}
